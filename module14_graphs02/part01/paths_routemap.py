...
from module13_graphs01.part02_basics02.graphs_etc import longest_shortest_path
...
def function_one(route_graph):
    ...
    """
    path = longest_shortest_path(route_graph)
    return path
...
def function_two(route_graph):
    ...
    l = {}
    home = "Miami"
    for node1 in route_graph.nodes():
        l1 = 0
        for node2 in route_graph.nodes():
            if node1 == node2:
                pass
            else:
                l2 = nx.shortest_path_length(route_graph, node1, node2)
                l1 = l1 + l2
            l[node1] = l1
    for key in l.keys():
        if l[home] > l[key]:
...
if __name__ == '__main__':
    ...
    print("Number of hops from Portland to Tulsa: {0}".format(len(nx.shortest_path(routemap_graph, "Portland", "Dallas")) - 1))
    print("Itinerary: {0}".format(nx.shortest_path(routemap_graph, "Portland", "Dallas")))
    print(function_one(routemap_graph))
    print(function_two(routemap_graph))
    #display_and_save(routemap_graph, "Flight")